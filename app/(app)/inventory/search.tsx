...
export default function SearchScreen() {
  ...
  function handleQueryChange(text: string) {
    ...
    debounceTimer.current = setTimeout(() => {
      fetchingRef.current = false; // liberta o lock para nova pesquisa
  ...
  async function handleResolveItem(dest: string) {
    ...
    try {
      ...
      setItems((prev) => prev.filter((i) => i.id !== resolveTargetItem.id));
    } catch {
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.header}>
        ...
        />
        {query.length > 0 && (
          <TouchableOpacity onPress={() => { setQuery(''); setItems([]); setHasMore(false); setExpandedItemId(null); }}>
